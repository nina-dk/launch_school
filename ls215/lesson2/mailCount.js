/*
input: string of data representing emails
output: string that contains the number of emails and the date range of those emails
rules:
  -each email consists of 5 parts: Sender, subject, date, recipient, body in that order
  -each email part is spearated from the next one by "#/#\n"
  -each email is separated from the next email by "##||##\n\n"
  -the dates are in the format MM-DD-YY
algorithm:
  -split the input string into an array by the email delimiter ("##||##\n\n")
  -for each produced string split it into an array of the email parts by ("#/#\n")
  -get the length of the array of emails (count)
  -sort the emails in the array by date in ascending order
  -get the first and last date, which designate the date range
*/
...
function mailCount(emailData) {
  ...
  let dates = emails.map(email => {
    ...
    let date = parts.find(part => part.startsWith("Date"));
    return new Date(date.match(/[0-9\-]+/)[0]);
  });
