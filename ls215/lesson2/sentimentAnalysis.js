...
//determine how many positive and how many negative words exist in the input text
//compare the totals and give a verdict based on the comparison
//if a word is part of another word, it doesn't count
function sentiment(text) {
  let words = text.replaceAll(/[^\w ]/g, " ")
                  .toLowerCase()
                  .split(" ")
                  .filter(word => word);
  ...
  else if (negatives.length > positives.length) sentiment = "Negative";
...
function sentiment(text) {
  ...
  let positiveWords = words.filter(word => positiveRegex.test(word));
  let negativeWords = words.filter(word => negativeRegex.test(word));
  let sentiment = "";
  ...
  console.log(`There are ${positiveWords.length} positive words in the text.\n` +
    ...
    `There are ${negativeWords.length} negative words in the text.\n` +
