...
describe("CustomSet", () => {
  describe("set must be comprised of unique elements", () => {
    test("default value is an empty set", () => {
      let set = new CustomSet();
      expect(set.isEmpty()).toBe(true);
    });

    test("unique elements are added to new set", () => {
      let set = new CustomSet([1, 2, 3]);
      expect(set.elements).toEqual([1, 2, 3]);
    });

    test("duplicates are only added once to set", () => {
      let set1 = new CustomSet([1, 2, 1, 3, 2, 4]);
      let set2 = new CustomSet([1, 2, 3, 4]);
      expect(set1.isSame(set2)).toBe(true);
    })
  });
